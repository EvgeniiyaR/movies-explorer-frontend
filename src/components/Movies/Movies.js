...
const Movies = ({
  ...
  const [visibleCardsCount, setVisibleCardsCount] = useState(cardsToShowInitial);
  ...
  useEffect(() => {
    if (visibleCardsCount % cardsToShow !== 0) {
      setVisibleCardsCount(visibleCardsCount + (cardsToShow - (visibleCardsCount - (Math.floor(visibleCardsCount / cardsToShow) * cardsToShow))));
    }
  }, [cardsToShow]);
