...
const Profile = ({ handleSignOut, status, setStatus, isLoading, handleUpdateUser, isEdit, setIsEdit }) => {
  ...
  const validationRules = {
    ...
    [
      ...
      {
        regex: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
        message: 'Поле может содержать адрес электронной почты (например, test@test.test)',
      },
    ],
  };

  const { values, handleChange, errors } = useFormWithValidation(initialValues, validationRules);
      ...
      <section className="profile">
        ...
        <Form
          ...
          <div className="profile__container-inputs">
            <span className="profile__input-error profile__input-error_type_name">{errors.name}</span>
            <Input
              classNameInput={`profile__input ${errors.name ? "profile__input_type_error" : ""}`}
              ...
              max="30" />
            <Input
              classNameInput={`profile__input ${errors.email ? "profile__input_type_error" : ""}`}
            ...
            {isEdit ?
              <>
                ...
                <Button
                  ...
                  type="submit"
                  text="Сохранить"
                  disabled={(errors.name || errors.email || (values.name === currentUser.name && values.email === currentUser.email) || isLoading) && true} />
              </>
              :
              <>
                <Button className="profile__button profile__button_type_edit" type="submit" text="Редактировать" />
                <Button className="profile__button profile__button_type_logout" type="button" text="Выйти из аккаунта" onClick={handleSignOut} />
              </>
            }
          </div>
        </Form>
